...
class Order(models.Model):
    ...
    order_total = models.DecimalField(max_digits=10, decimal_places=2, null=False, blank=False, default=0)
    grand_total = models.DecimalField(max_digits=10, decimal_places=2, null=False, blank=False, default=0)
    original_bag = models.TextField(null=False, blank=False, default='')
    stripe_pid = models.TextField(max_length=254, null=False, blank=False, default='')


    def _generate_order_number(self):
        """Generate a rendom, unique order number using UUID"""
        return uuid.uuid4().hex.upper()

    def update_total(self):
        """
        Update Grand total each time a line item is added,
        accounting for the delivery costs
        """
